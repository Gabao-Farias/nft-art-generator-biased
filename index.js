...
const template = `
    <svg width="2111" height="2655" viewBox="0 0 2111 2655" fill="none" xmlns="http://www.w3.org/2000/svg">
        <!-- bg -->
...
function getLayer(name, skip=0.0) {
    const svg = readFileSync(`./my-layers/${name}.svg`, 'utf-8');
...
function createImage(idx) {
    ...
    const eyes = randInt(5);
    const nose = randInt(5); 
    const mouth = randInt(4);
    const beard = randInt(5);
    // 18,900 combinations

    const face = [eyes, mouth, nose, beard].join('');
